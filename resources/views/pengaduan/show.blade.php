    ...
    <div class="bg-white shadow rounded-lg p-6">
        ...
        <div class="mt-8">
            ...
            <form method="POST" action="/pengaduan/{{ $pengaduan->id }}/tanggapan">
                @csrf
                <div class="mb-4">
                    <textarea name="isi_tanggapan" rows="4" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500 @error('isi_tanggapan') border-red-500 @enderror" placeholder="Tulis tanggapan Anda di sini...">{{ old('isi_tanggapan') }}</textarea>
                    @error('isi_tanggapan')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                <div class="flex justify-end">
                    <button type="submit" class="px-4 py-2 bg-indigo-600 text-white rounded hover:bg-indigo-700 transition duration-200 transform hover:scale-105 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
        ...
        @if($pengaduan->tanggapan->count() > 0)
            <div class="mt-8">
                ...
                @foreach($pengaduan->tanggapan as $tanggapan)
                    <div class="p-4 {{ $tanggapan->admin_id ? 'bg-indigo-50' : ($tanggapan->user_id === $pengaduan->user_id ? 'bg-green-50' : 'bg-gray-50') }} rounded mb-4">
                        <div class="flex items-start space-x-3">
                            <div class="flex-shrink-0">
                                @if($tanggapan->admin_id)
                                    <img class="h-8 w-8 rounded-full object-cover" src="{{ $tanggapan->admin->getProfilePhotoUrl() }}" alt="{{ $tanggapan->admin->name ?? 'Admin' }}'s Avatar">
                                @else
                                    <img class="h-8 w-8 rounded-full object-cover" src="{{ $tanggapan->user->getProfilePhotoUrl() }}" alt="{{ $tanggapan->user->name ?? 'User' }}'s Avatar">
                                @endif
                            </div>
                            <div class="flex-1">
                                ...
                                <div class="text-xs text-gray-500">
                                    @if($tanggapan->admin_id)
                                        <span class="font-semibold text-indigo-600">Admin:</span> {{ $tanggapan->admin->name ?? 'Admin' }}
                                    @elseif($tanggapan->user_id === $pengaduan->user_id)
                                        <span class="font-semibold text-green-600">Pelapor:</span> {{ $tanggapan->user->name ?? 'User' }}
                                    @else
                                        <span class="font-semibold text-gray-600">User:</span> {{ $tanggapan->user->name ?? 'User' }}
                                    @endif
