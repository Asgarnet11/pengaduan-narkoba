...
<nav x-data="{ open: false }" class="sticky top-0 z-50 bg-white shadow-md backdrop-blur supports-backdrop-blur:bg-white/95">
        ...
        <div class="flex justify-between h-16">
            ...
            <div class="flex items-center">
                <div class="shrink-0 flex items-center">
                    <a href="{{ route('dashboard') }}" class="flex items-center space-x-3 group">
                        <i class="fas fa-building text-2xl text-indigo-600 group-hover:scale-110 transition-transform duration-300"></i>
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ml-10 sm:flex">
                    ...
                    <a href="{{ route('dashboard') }}"
                        @class([
                            'inline-flex items-center px-1 pt-1 text-sm font-medium transition-all duration-300',
                            'text-gray-900 border-b-2 border-indigo-500' => request()->routeIs('dashboard'),
                            'text-gray-500 hover:text-gray-700 hover:border-b-2 hover:border-gray-300' => !request()->routeIs('dashboard')
                        ])>
                    ...
                    <a href="{{ route('pengaduan.index') }}"
                        @class([
                            'inline-flex items-center px-1 pt-1 text-sm font-medium transition-all duration-300',
                            'text-gray-900 border-b-2 border-indigo-500' => request()->routeIs('pengaduan.*'),
                            'text-gray-500 hover:text-gray-700 hover:border-b-2 hover:border-gray-300' => !request()->routeIs('pengaduan.*')
                        ])>
                    ...
                    @if($isAdmin)
                        <a href="{{ route('admin.dashboard') }}"
                            @class([
                                'inline-flex items-center px-1 pt-1 text-sm font-medium transition-all duration-300',
                                'text-gray-900 border-b-2 border-indigo-500' => request()->routeIs('admin.*'),
                                'text-gray-500 hover:text-gray-700 hover:border-b-2 hover:border-gray-300' => !request()->routeIs('admin.*')
                            ])>
            ...
            <div class="hidden sm:flex sm:items-center sm:ml-6 space-x-4">
                ...
                <div class="ml-3 relative" x-data="{ open: false }">
                    <div>
                        <button @click="open = !open" class="flex items-center space-x-3 text-sm focus:outline-none transition duration-300 ease-in-out hover:opacity-80">
                    ...
                    <div x-show="open"
                        ...
                        x-transition:leave-end="opacity-0 transform scale-95">
                        <div class="py-1">
                            <a href="{{ route('profile.edit') }}" class="group flex items-center px-4 py-2 text-sm text-gray-700 hover:bg-indigo-50 transition-all duration-300">
                                <i class="fas fa-user mr-3 text-gray-400 group-hover:text-indigo-500"></i>
                                Profile
                            </a>
                            <form method="POST" action="{{ route('logout') }}">
                                @csrf
                                <button type="submit" class="w-full group flex items-center px-4 py-2 text-sm text-gray-700 hover:bg-indigo-50 transition-all duration-300">
                ...
                <div class="flex space-x-4">
                    <a href="{{ route('login') }}" class="inline-flex items-center px-4 py-2 text-sm font-medium text-indigo-600 bg-white border border-indigo-600 rounded-lg hover:bg-indigo-50 transition-all duration-300">
                        <i class="fas fa-sign-in-alt mr-2"></i>
                        Login
                    </a>
                    <a href="{{ route('register') }}" class="inline-flex items-center px-4 py-2 text-sm font-medium text-white bg-indigo-600 border border-transparent rounded-lg hover:bg-indigo-700 transition-all duration-300">
